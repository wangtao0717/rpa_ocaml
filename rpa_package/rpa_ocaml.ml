...
(**left click a element  if find it by the keyword or coordinate*)
  let click element_or_coordinate = 
    match element_or_coordinate with 
    |Element element -> let element = Py.String.of_string element in
                  Py.Module.get_function r "click" [|element|];();
    |Coordinate(x,y) -> let coordinate =  Array.map [|x;y|] Py.Int.of_int in
                  Py.Module.get_function r "click" coordinate;();
  ;;


(**right click a element if find it by the keyword or coordinate*)
  let rclick element_or_coordinate = 
    match element_or_coordinate with 
    |Element element -> let element = Py.String.of_string element in
...
(**double click a element if find it by the keyword or coordinate*)
  let dclick element_or_coordinate = 
    match element_or_coordinate with 
    |Element element -> let element = Py.String.of_string element in
...
(**move mouse to a element if find it by the keyword or coordinate*)
  let hover element_or_coordinate = 
    match element_or_coordinate with 
    |Element element -> let element = Py.String.of_string element in
...
(**	enter text at element if find it by the keyword or coordinate*)
  let type_text element_or_coordinate text= 
    let text = Py.String.of_string text in
    match element_or_coordinate with 
    |Element element -> let element = Py.String.of_string element in
                  Py.Module.get_function r "type" [|element;text|];();
    |Coordinate(x,y) -> let x = Py.Int.of_int x in
                        let y = Py.Int.of_int y in
                  Py.Module.get_function r "type" [|x;y;text|];();
  ;;

(*choose dropdown option*)

  let select element option_value =
    match element with 
    |Element element -> let element = Py.String.of_string element in
        match option_value with 
        |Element option_value -> let option_value = Py.String.of_string option_value in
                                Py.Module.get_function r "select" [|element;option_value|];();
        |Coordinate (x_option,y_option) -> let x_option = Py.Int.of_int x_option in
                                          let y_option = Py.Int.of_int y_option in
                                          Py.Module.get_function r "select" [|element;x_option;y_option|];();                                
    |Coordinate(x_element,y_element) -> let x_element = Py.Int.of_int x_element in
                                        let y_element = Py.Int.of_int y_element in
        match option_value with 
        |Element option_value -> let option_value = Py.String.of_string option_value in
                                Py.Module.get_function r "select" [|x_element;y_element;option_value|];();
        |Coordinate (x_option,y_option) -> let x_option = Py.Int.of_int x_option in
                                          let y_option = Py.Int.of_int y_option in
                                          Py.Module.get_function r "select" [|x_element;y_element;x_option;y_option|];();  
  ;;                      

  (**fetch & return element text *)
  let read range_of_element = 
    match range_of_element with 
    |Element_range element -> let element = Py.String.of_string element in
                  let result = Py.Module.get_function r "read" [|element|] in
                  Py.String.to_string result
    |Range(x1,y1,x2,y2) -> let input = Array.map [|x1;y1;x2;y2|] Py.Int.of_int in
